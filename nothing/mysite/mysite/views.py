...
def home(request):
    ...
    for i in platform:
        ...
        highest_bid = Bid.objects.filter(property=i).order_by('-amount').first()
        platform_bids[i.id] = highest_bid

    return render(request, 'home.html', {
        "platform": platform, 'platform_bids': platform_bids,
    })
...
def place_bid(request, platform_id):
    ...
    return render(request, 'home.html', {'form': form, 'platform': platform})
